...
#modified to work with cifar10 and batched
def project(
    G,
    target: torch.Tensor, # [B,C,H,W] and dynamic range [0,255], W & H must match G output resolution
    *,
    classes,
    num_steps = 1000,
    w_avg_samples = 10000,
    ...
    assert target.shape == (target.shape[0], G.img_channels, G.img_resolution, G.img_resolution)
    ...
    ws = torch.empty((target.shape[0], 1, 512), dtype=torch.float32, device=device)
    print(target.shape)
    for idx in  range(target.shape[0]):
        z_samples = torch.from_numpy(np.random.RandomState(123).randn(w_avg_samples, G.z_dim)).to(device)
        logits = torch.zeros((10, ), dtype=torch.float32, device=device)
        logits[classes[idx]] = 1.0
    ...
    # initalize optimizer
    w_opt = torch.tensor(ws, dtype=torch.float32, device=device, requires_grad=True) # pylint: disable=not-callable
    optimizer = torch.optim.AdamW([w_opt], betas=(0.9, 0.999), lr=initial_learning_rate)
    ...
    for step in range(num_steps):
        ...
        synth_images = G.synthesis(w_opt.repeat(1,G.num_ws,1), noise_mode=noise_mode)
    ...
    return synth_images, w_opt.detach()


#modified for cifar10 and batched
@click.command()
...
def run_projection(
    ...
    files = os.listdir(target_fname)

    targets = np.empty((len(files), 32, 32, 3), dtype=np.uint8)
    classes = np.empty((len(files),), dtype=np.uint8)
    names = []

    #load images
    for idx, file in enumerate(files):
        name, suf = file.split("_")
        c, _ = suf.split(".")
        names.append(name)
        print(name)
        print(c)
        print(target_fname + file)
        target_pil = PIL.Image.open(target_fname + file).convert('RGB')
        w, h = target_pil.size
        s = min(w, h)
        target_pil = target_pil.crop(((w - s) // 2, (h - s) // 2, (w + s) // 2, (h + s) // 2))
        target_pil = target_pil.resize((G.img_resolution, G.img_resolution), PIL.Image.LANCZOS)
        target_uint8 = np.array(target_pil, dtype=np.uint8)
        targets[idx] = target_uint8
    ...
    if not w_init:
        ...
        all_images, projected_w = project(
            G,
            target=torch.tensor(targets.transpose([0, 3, 1, 2]), device=device), # pylint: disable=not-callable
            classes=classes,
            num_steps=inv_steps,
            device=device,
            verbose=True,
    ...
    print(projected_w[0].repeat(1, G.num_ws, 1).shape)
    exit()
    for i in range(targets.shape[0]):
        target_pil.save(f'{outdir}/{names[i]}target.png')
        synth_image = G.synthesis(projected_w[i].repeat(1, G.num_ws, 1))
        synth_image = (synth_image + 1) * (255/2)
        synth_image = synth_image.permute(0, 2, 3, 1).clamp(0, 255).to(torch.uint8)[0].cpu().numpy()
        PIL.Image.fromarray(synth_image, 'RGB').save(f'{outdir}/{names[i]}proj.png')
